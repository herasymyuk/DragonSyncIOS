...
def wait_for_device(device_path, timeout=None):
    """Wait for the device to become available with optional timeout"""
    start_time = time.time()
    print(f"Waiting for device: {device_path}")
    
    while True:
        if os.path.exists(device_path):
            print(f"Device {device_path} found")
            return True
        
        if timeout and (time.time() - start_time > timeout):
            print(f"Timeout waiting for device: {device_path}")
            return False
        
        print(f"Device {device_path} not found, waiting...")
        time.sleep(1)
...
def main():
    ...
    print(f"Serial Monitor Service starting...")
    
    # Wait for device to be available
    timeout = args.device_timeout if args.device_timeout > 0 else None
    if not wait_for_device(args.device, timeout):
        print("Device not found within timeout period. Exiting.")
        return 1
    ...
    try:
        ...
        while True:
            try:
                ...
                time.sleep(5)
                if not wait_for_device(args.device, 30):  # 30 second timeout for reconnect
                    print("Failed to reconnect to device. Exiting.")
                    break
                try:
                    ser = serial.Serial(
                        port=args.device,
                    ...
                    print(f"Reconnected to serial port: {ser.name}")
                except serial.SerialException as e:
